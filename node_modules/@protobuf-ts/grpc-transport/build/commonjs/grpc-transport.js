...
class GrpcTransport {
    ...
    unary(method, input, options) {
        ...
        const gCall = this.client.makeUnaryRequest(`/${method.service.typeName}/${method.name}`, (value) => Buffer.from(method.I.toBinary(value, opt.binaryOptions)), (value) => method.O.fromBinary(value, opt.binaryOptions), input, gMeta, this.pickCallOptions(opt), (err, value) => {
            ...
            if (err) {
                const e = new runtime_rpc_1.RpcError(err.message, grpc_js_1.status[err.code], util_1.metadataFromGrpc(err.metadata));
    ...
    serverStreaming(method, input, options) {
        ...
        gCall.addListener('error', err => {
            const e = util_1.isServiceError(err) ? new runtime_rpc_1.RpcError(err.message, grpc_js_1.status[err.code], util_1.metadataFromGrpc(err.metadata)) : new runtime_rpc_1.RpcError(err.message);
            e.methodName = method.name;
            e.serviceName = method.service.typeName;
            defHeader.rejectPending(e);
            if (!outStream.closed) {
