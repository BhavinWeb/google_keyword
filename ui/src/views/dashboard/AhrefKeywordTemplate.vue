		...
		<div class="flex flex-col overflow-x-hidden overflow-y-scroll">
			<div class="overflow-x-auto overflow-y-auto">
				...
				<div class="p-1.5 w-full inline-block align-middle">
					...
					<AhKeywordTable v-if="!hideTable"
									:keyword-set-ah="ahrefResult" :searched-keys="searchedKeywords"
									:selected-ah="selectedKeywordAh" :template-list="templateList"
									@addToTemplate="addKeywordsToTemplate" @displayResult="changeResultTable"></AhKeywordTable>
				</div>
			</div>
		</div>
	</div>
	...
	methods: {
		async getRegions() {
			...
			await axios.post(this.$baseURL + '/GetCountries').then(response => {
				...
				if (this.countrySelected === 0) {
					this.countrySelected = locations.filter(obj => obj.country_code === this.defaultCountryCode).map(obj => obj.criteria_id)[0]
					// this.getProvinces();
		...
		/*async searchKeyword() {
			if((!this.templateOption && this.keywords != '') || (this.templateOption && this.templateSelected.length)){
				...
				await axios.post(this.$baseURL + '/SearchKeyword', data).then(response => {
